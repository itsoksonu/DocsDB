...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!S3Manager.isValidFileType(fileType, fileName, uploadType)) {
        return res.status(400).json({
          success: false,
      ...
      const folderPrefix = uploadType === "avatar" ? "avatars" : "uploads";
      const s3Key = S3Manager.generateFileKey(userId, fileName, folderPrefix);

      const presignedUrl = await S3Manager.generatePresignedUrl(
        s3Key,
        fileType,
        fileSize
      );
...
// Get upload status
router.get("/status/:documentId", authMiddleware, async (req, res, next) => {
  try {
